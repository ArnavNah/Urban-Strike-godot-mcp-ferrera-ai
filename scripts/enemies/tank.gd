...
func _fire_mortar_shell() -> void:
	...
	inner_cyl.top_radius = 0.2
	inner_cyl.bottom_radius = 0.2
	inner_cyl.height = 0.12
	var inner_mat := StandardMaterial3D.new()
	inner_mat.albedo_color = Color(1.0, 0.85, 0.2, 0.80)
	inner_mat.transparency = BaseMaterial3D.TRANSPARENCY_ALPHA
	inner_mat.shading_mode = BaseMaterial3D.SHADING_MODE_UNSHADED
	inner_inst.mesh = inner_cyl
	inner_inst.material_override = inner_mat
	warning_node.add_child(inner_inst)

	var ring_tween := warning_node.create_tween()
	if ring_tween:
		ring_tween.tween_property(inner_inst, "scale", Vector3(27.5, 1.0, 27.5), 1.6)
...
func take_damage(amount: float, _source: Node = null, _hit_pos: Vector3 = Vector3.ZERO) -> void:
	...
	if actual_damage > 0.0:
		...
		if eb and eb.has_signal("damage_number_spawned"):
			eb.emit_signal("damage_number_spawned", global_position + Vector3(0, _get_damage_number_y_offset(), 0), actual_damage, false, {"target_id": get_instance_id(), "is_lethal": current_health <= 0.0})
...
func _die() -> void:
	...
	# Transition visual meshes to charred wreckage that lingers cleanly
	...
	if tw:
		...
		tw.tween_property(self, "position:y", position.y - 0.5, 0.6)
