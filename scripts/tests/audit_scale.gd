...
func _init() -> void:
	...
	for category: String in targets.keys():
		...
		for item_name: String in cat_items.keys():
			...
			# Calculate AABB relative to inst transform
	...
	var fa := FileAccess.open("res://scale_audit.txt", FileAccess.WRITE)
	if fa:
		for line: String in out:
			fa.store_line(line)
			print(line)
		fa.close()
		print("Wrote scale_audit.txt successfully!")
	else:
		print("ERROR: Failed to open scale_audit.txt for writing!")
...
func _calculate_node_visual_aabb(node: Node, root_xform: Transform3D) -> AABB:
	...
	while not stack.is_empty():
		...
		# Exclude top_level decals/shadows/particles like GroundShadow or DownwashDust
		...
		if curr is VisualInstance3D:
			...
			if a.size.length_squared() > 0.0001:
				# Transform aabb into root node space
