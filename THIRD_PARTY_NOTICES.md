# Third-Party Notices and Licenses

This project incorporates third-party open source assets under the following licenses:

---

## 1. Fonts

### Rajdhani (SemiBold, Bold)
- **Author**: Indian Type Foundry (Satya Rajpurohit, Jyotish Sonowal)
- **Source**: Google Fonts / Indian Type Foundry
- **License**: SIL Open Font License, Version 1.1 (OFL-1.1)
- **License File**: `res://assets/licenses/OFL-Rajdhani.txt`

### Inter (Regular, SemiBold)
- **Author**: Rasmus Andersson
- **Source**: rsms/inter / Google Fonts
- **License**: SIL Open Font License, Version 1.1 (OFL-1.1)
- **License File**: `res://assets/licenses/OFL-Inter.txt`

---

## 2. Icons

### Lucide Icons (play, wrench, settings, power, chevron-right)
- **Author**: Lucide Contributors
- **Source**: https://github.com/lucide-icons/lucide
- **License**: ISC License
- **License File**: `res://assets/licenses/LICENSE-Lucide.txt`

---

## 3. Audio

### Kenney Interface Sounds (ui_focus, ui_confirm, ui_back)
- **Author**: Kenney (Asset pack: Interface Sounds)
- **Source**: https://kenney.nl/assets/interface-sounds / https://github.com/Calinou/kenney-interface-sounds
- **License**: Creative Commons Zero v1.0 Universal (CC0 1.0)
- **License File**: `res://assets/licenses/LICENSE-Kenney.txt`

---

## 4. 3D Models & Modular Tiles

### Kenney 3D Road Tiles
- **Author**: Kenney (Kenney Vleugels - www.kenney.nl)
- **Source**: https://kenney.nl/assets/3d-road-tiles
- **License**: Creative Commons Zero v1.0 Universal (CC0 1.0)
- **License File**: `res://assets/licenses/LICENSE-Kenney-Road-Tiles.txt` (and `res://assets/licenses/LICENSE-Kenney-Road-Tiles.txt`)
- **Notice**: You may use these models/textures in personal and commercial projects. Credit is appreciated but not mandatory.


### Kenney City Kit Industrial
- **Author**: Kenney (Kenney Vleugels - www.kenney.nl)
- **Source**: https://kenney.nl/assets/city-kit-industrial
- **License**: Creative Commons Zero v1.0 Universal (CC0 1.0)
- **License File**: `res://assets/licenses/LICENSE-Kenney-City-Kit-Industrial.txt`

### Kenney City Kit Suburban
- **Author**: Kenney (Kenney Vleugels - www.kenney.nl)
- **Source**: https://kenney.nl/assets/city-kit-suburban
- **License**: Creative Commons Zero v1.0 Universal (CC0 1.0)
- **License File**: `res://assets/licenses/LICENSE-Kenney-City-Kit-Suburban.txt`
